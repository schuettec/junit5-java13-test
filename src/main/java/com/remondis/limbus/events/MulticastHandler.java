...
/**
 * Implementation of an {@link EventMulticaster} that uses {@link Proxy}-classes
 * to multicast event calls to subscribers.
 * <p>
 * <b>Note: The {@link EventMulticaster} can be configured to act in to
 * different ways:
 * <ul>
 * <li>If <code>throwException</code> is <code>false</code> and a subscriber
 * throws an exception this only creates a warning in the logs.</li>
 * <li>If <code>throwException</code> is <code>true</code></li> a potential
 * exception will be re-thrown by the multicast event method. The processing of
 * further subscribers is then aborted and not all subscribers may be
 * notified.</li>
 * </p>
 *
 * @author schuettec
...
final class MulticastHandler<I> implements EventMulticaster<I> {
		...
		public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
			...
			while (it.hasNext()) {
				...
				try {
					...
					// Translate to the cause of the invocation target exception because thats the
					// business logic exception.
					throw e.getCause();
				} catch (Exception e) {
					String message = String.format("Cannot multicast event to subscriber of type %s",
	...
	protected I createMulticasterProxy(Class<I> subscriberInterface, InvocationHandler handler) {
		...
		Object proxy = Proxy.newProxyInstance(classLoader, new Class[] { subscriberInterface }, handler);
